...
安装后默认建立空的个人库，不自动写入示例或其他用户的数据。首次调用时先运行 `status`；若返回 `library_empty: true`，必须明确说明空库是正常状态，并在当前回复末尾提供至少一种可直接复制的收藏方式：
...
也可以提示用户上传包含 Prompt 的截图并说“识别这张截图里的图片处理提示词，先核对完整文字，再收进图片库”，或在一次满意的实际处理后说“把刚才这个效果收进图片库”。不要要求用户理解字段、ID 或数据库；名称、分类、标签、适用场景等安全元数据由模型提取。只要正式库仍为空，就保留这条简短引导，避免用户安装后不知道如何开始。

如果当前消息同时附有待处理图片，空库不代表可以直接猜测风格并执行。先基于图片建立一个完整临时方案，把它作为带编号的 `Txxx` 选项展示，然后停止等待选择；同时附上上述收藏提示。临时方案仍须在用户明确选定、实际执行成功并认可后才能收藏。
...
空库时使用 `status` 返回的 onboarding 指引，明确告诉用户库为空，并给出粘贴文字、上传 Prompt 截图或收藏刚才效果的可复制说法。
